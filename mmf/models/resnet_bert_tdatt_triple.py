...
from mmf.modules.layers import ModalCombineLayer, ReLUWithWeightNormFC
...
@registry.register_model("resnet_bert_tdatt_triple")
class ResnetBERTTDAttTriple(BaseModel):
    """ResnetBERTTDAttTriple is a model for vision and language tasks with simple image and language encoders paired
    with Top-down attention fusion strategy. Different from ResnetBERTTDAtt model, this one concatenates
    original image features, attended image features and linguistic features.
    ...
    @classmethod
    def config_path(cls):
        return "configs/models/resnet_bert_tdatt_triple/defaults.yaml"
    ...
    def forward(self, sample_list):
        ...
        aligned_text_features = self.image_language_align(text_features)

        image = sample_list.image
        image_features_pool, image_features_grid = self.vision_module(image)
        attened_image_features, _ = self.process_feature_embedding(
            "image", image_features_grid, aligned_text_features
        )

        joint_embedding = self.combine_embeddings(
            ["image", "text"], [attened_image_features, aligned_text_features]
        )

        if image_features_pool.size(dim=0) == self.config.image_feature_dim:
            image_features_pool = image_features_pool.unsqueeze(0)

        triple_embedding = torch.cat([text_features, image_features_pool, joint_embedding], dim=1)

        o1_logits = self.classifier_o1(triple_embedding)
        o2_logits = self.classifier_o2(triple_embedding)
        s1_logits = self.classifier_s1(triple_embedding)
